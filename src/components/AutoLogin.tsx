  ...
  useEffect(() => {
    const tryAutoLogin = async () => {
      ...
      if (nostr?.peekPublicKey) {
        try {
          debugLog('[AutoLogin] Checking for auto-login via peekPublicKey...');
          const pubkey = await nostr.peekPublicKey();

          if (pubkey) {
            debugLog('[AutoLogin] Found pubkey via peekPublicKey, auto-logging in:', pubkey);
          ...
          debugLog('[AutoLogin] Error calling peekPublicKey:', error);
        }
      }
    };
